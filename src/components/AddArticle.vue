<template>
  <div class="add-article">
    <el-form ref="add-article-form" :model="form" label-width="80px" :rules="rules">
      ...
      <el-form-item class="add-buttons">
        <el-button type="primary" @click="onSubmit">新增文章</el-button>
...
export default {
  ...
  methods: {
    onSubmit() {
      this.$refs['add-article-form'].validate((valid) => {
        if (valid) {
          let formData = {};
          ...
          axios.post(this.$store.state.API_HOST + '/articles', formData)
            .then((res)=> {
